# 🚀 NextGenLAB - NASA Uzay Biyobilim Keşif Platformu
## NASA Uzay Biyolojisi Yayınları için Yapay Zeka Destekli Araştırma Platformu
...
### Nasıl Çalışır
Sistem, kullanıcının doğal dil sorgusunu vektör embedding'e dönüştürür ve FAISS index kullanarak en benzer makaleleri bulur. Sonuçlar ilgi skoruna göre sıralanır.

---

## 2. 🤖 Yapay Zeka Destekli Soru-Cevap

### Devrim Niteliğinde Özellik

Araştırma makaleleri hakkında doğal sorular sorun ve doğru, bağlamsal cevaplar alın.
...
```typescript
// Frontend: Soru ve dil tercihi gönder
const res = await fetch(`${api}/qa`, {
  method: "POST",
  body: JSON.stringify({ 
    id, 
    question, 
    language: lang // 'en' veya 'tr'
  })
});

// Backend: Makaleyi çek ve işle
...
const aiResponse = await openai.chat.completions.create({
  ...
  messages: [{
    role: "system",
    content: `${language} dilinde cevapla, SADECE şu içeriğe dayanarak: ${articleContent}`
  }]
});
```
...
### Kullanım Senaryosu
Bir araştırmacı 5 makaleyi seçip "Özetle" butonuna tıkladığında, sistem GPT-4o-mini kullanarak tüm makalelerin ana bulgularını, metodolojilerini ve sonuçlarını özetler.
...
Araştırma varlıkları arasındaki ilişkileri keşfedin.
...
- Cytoscape.js (Graf Görselleştirme)
- CSS3 (Özel Animasyonlar ve Degradeler)
```
...
- Sentence Transformers (Embedding'ler)
- FAISS (Vektör Arama)
```

### AI Servisleri
```
- OpenAI GPT-4o-mini (Soru-Cevap ve Özetleme)
- sentence-transformers/all-MiniLM-L6-v2 (Embedding'ler)
- NCBI E-utilities API (Makale İçeriği)
```
...
### Ana Sayfa (page.tsx) - 740 satır

```typescript
// State Yönetimi
const [q, setQ] = useState(""); // Arama sorgusu
const [items, setItems] = useState<Item[]>([]); // Sonuçlar
const [lang, setLang] = useState<"tr" | "en">("en"); // Dil
...
const generateSmartSuggestions = useCallback((query: string) => {
  ...
  // Dil tespit et ve önerileri filtrele
...
// Makale İçeriği ile AI Soru-Cevap
...
**Akıllı Arama Dropdown**:
```typescript
// Maksimum z-index ile konumlandırıldı
<div style={{ 
  position: "absolute",
  top: "calc(100% + 8px)",
  zIndex: 2147483647,
...
async def search(q: str):
    """Embedding'ler kullanarak semantik arama"""
...
async def qa_endpoint(id: int, question: str, language: str):
    """Makale içeriği çekme ile Soru-Cevap"""
    # PMC ID çıkar
    ...
    # XML'i parse et ve içeriği çıkar
    ...
    TALİMATLAR:
    - {language} dilinde cevapla
    - SADECE sağlanan içeriği kullan
    - Gerçeklere dayalı ol
    """
...
CREATE TABLE publications (
    ...
    authors TEXT[],
    url TEXT,
    embedding VECTOR(384)  -- Semantik arama için
);
...
## Soru-Cevap API Rotası (route.ts) - Geliştirilmiş Versiyon
...
export async function POST(req: NextRequest) {
  ...
  if (pmcId) {
    ...
    // Regex kullanarak bölümleri çıkar
...
KRİTİK TALİMATLAR:
- SADECE makale içeriğine dayanarak cevapla
- ${language === 'en' ? 'İNGİLİZCE' : 'TÜRKÇE'} dilinde yaz
- Bilgi bulunamazsa belirt: "${language === 'en' ? 'Bu bilgi makalede yok' : 'Bu bilgi makalede yok'}"
- Spesifik bulguları alıntıla
- Bilgi uydurma
    `;
  }
...
## Bilgi Grafiği Bileşeni (KGDashboard.jsx)
...
AI Özetleme: < 5s
Soru-Cevap Yanıtı: < 10s (makale çekme dahil)
Bilgi Grafiği Yükleme: < 3s
...
```
- İndekslemeli PostgreSQL: 100K+ yayını destekler
- FAISS vektör arama: Saniye altı benzerlik araması
- FastAPI async: 1000+ eşzamanlı istek işler
- Next.js ISR: Hızlı yüklemeler için statik yeniden oluşturma
- Vercel Edge: Global CDN dağıtımı
```
...
**Kullanım Senaryosu**: Literatür taraması ve hipotez oluşturma
- Belirli araştırma konularını arama
- Kapsamlı özetler oluşturma
- Metodolojiler hakkında detaylı sorular sorma
- İlgili çalışmalar için bilgi grafiğini keşfetme

**Örnek İş Akışı**:
```
1. Arama: "mikrogravitede kemik kaybı mekanizmaları"
2. Filtrele: Yıl 2015-2024, Organizma: İnsan
3. İncele: İlgi skoruna göre ilk 10 sonuç
4. Soru-Cevap: "Osteoblast aktivitesi hakkında önemli bulgular neler?"
5. Özetle: 3 makalenin bilim insanı odaklı özetini oluştur
6. Keşfet: İlgili deneyler için bilgi grafiğini kontrol et
```

## 2. 💼 Yönetici

**Kullanım Senaryosu**: Yatırım fırsatı değerlendirmesi
- Araştırma trendlerini belirleme
- Fon etkilerini değerlendirme
- Yayın çıktılarını karşılaştırma
- Stratejik karar verme

**Örnek İş Akışı**:
```
1. Analitik: Yayın zaman çizelgesini görüntüle (son 10 yıl)
2. Filtrele: Platform: ISS, Yıl: 2020-2024
3. Karşılaştır: Organizma tipine göre araştırma çıktısı
4. Soru-Cevap: "Son ISS çalışmalarının ana sonuçları neler?"
5. Rapor: Paydaş sunumu için bulguları dışa aktar
```

## 3. 🏗️ Misyon Mimarı

**Kullanım Senaryosu**: Güvenli keşif stratejisi geliştirme
- Bilgi boşluklarını belirleme
- Yayınlanmış araştırmalardan risk değerlendirmesi
- Teknoloji hazırlık seviyesi değerlendirmesi
- Misyon planlama içgörüleri

**Örnek İş Akışı**:
```
1. Arama: "uzun süreli uzay uçuşu sağlık riskleri"
2. Filtrele: Platform: ISS + Shuttle, Tüm yıllar
3. Bilgi Grafiği: Risk bağlantılarını keşfet
4. Soru-Cevap: "Kas atrofisi için hangi önlemler test edildi?"
5. Özetle: Misyon odaklı brifing oluştur
```
...
```
✓ NASA SB Publications GitHub (608 makale)
✓ PubMed Central (PMC) - Tam metin erişimi
✓ NCBI E-utilities API - Makale içeriği
✓ NASA OSDR - Uzay biyolojisi verisi
✓ NASA Task Book - Proje veritabanı
```
...
const limiter = rateLimit({
  ...
  max: 100 // her IP için windowMs başına 100 istek limiti
});
...
// Ortam Değişkenleri
const config = {
  apiKey: process.env.OPENAI_API_KEY, // İstemciye asla açılmaz
  dbUrl: process.env.DATABASE_URL,    // Sadece sunucu tarafı
  apiUrl: process.env.NEXT_PUBLIC_API_URL // Herkese açık, güvenli
};
```

---

# 🌟 FARKLILAŞTIRAN ÖZELLİKLER

## NextGenLAB'i Özel Kılan Nedir?

### 1. **Gerçek Makale İçeriği Analizi**
...
// Lint-Free Kod Tabanı
// ✓ ESLint: 0 hata
// ✓ TypeScript: 0 tip hatası
// ✓ Build: Başarılı
```
...
## Öğrenciler ve Araştırmacılar İçin

```
✓ 608 NASA yayınına ücretsiz erişim
✓ Uzay biyolojisi araştırmaları hakkında bilgi edinin
✓ AI destekli araştırma araçlarını anlayın
✓ Bilgi grafiği ilişkilerini keşfedin
✓ Bilimsel literatür taraması pratiği yapın
```

## Kurumlar İçin

```
✓ Araştırma yöntemleri için eğitim aracı
✓ Bilimde AI'nın gösterimi
✓ Bilgi yönetimi platformu
✓ Ekipler için işbirliği merkezi
✓ Toplumsal erişim ve katılım
```
...
✅ 95+ Lighthouse skoru
✅ Tam erişilebilirlik (WCAG 2.1)
✅ Mobil optimize
✅ Production hazır dağıtım
```

## Yenilik Öne Çıkanlar

```
🌟 Soru-Cevap için NCBI PMC içerik çekmeyi entegre eden ilk platform
🌟 En kapsamlı NASA uzay biyolojisi bilgi grafiği
🌟 Gelişmiş çok dilli AI yanıtları (sadece çeviri değil)
🌟 Gerçek zamanlı akıllı arama önerileri
🌟 Persona seçimi ile bağlam duyarlı özetleme
```

---

# 📞 İLETİŞİM VE BAĞLANTILAR
...
### Faz 1 (Q1 2024)
```
□ Gelişmiş filtreleme (yazar, kurum, fon)
□ Dışa aktarma işlevi (PDF, CSV)
□ Alıntı yönetimi
□ İşbirlikçi notlar
```
...
### Faz 2 (Q2 2024)
```
□ Makine öğrenimi trend tahminleri
□ Otomatik literatür tarama raporları
□ Daha fazla veritabanı ile entegrasyon
□ Üçüncü taraf geliştiriciler için API
```
...
□ Çevrimdışı mod
□ Gelişmiş analitik (ML destekli içgörüler)
□ Topluluk özellikleri (tartışmalar, incelemeler)
```
...
```
✅ Doğru: Gerçek makale içeriği, sıfır AI halüsinasyonu
✅ Hızlı: < 500ms arama, < 5s özetler
✅ Kapsamlı: 608 makale, 3,107 düğüm, 40,967 bağlantı
✅ Akıllı: GPT-4o-mini destekli Soru-Cevap ve özetleme
✅ Erişilebilir: Çok dilli, mobil dostu, sezgisel arayüz
✅ Açık: Herkes için NASA araştırmalarına ücretsiz erişim
```
...
### How It Works
The system converts user's natural language query into vector embeddings and uses FAISS index to find the most similar articles. Results are ranked by relevance score.
...
Ask natural questions about research articles and get accurate, contextual answers.
...
### Use Case
A researcher selects 5 articles and clicks "Summarize" button. The system uses GPT-4o-mini to summarize key findings, methodologies, and results from all articles.
...
Explore relationships between research entities.
